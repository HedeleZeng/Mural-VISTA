...
#
# Modifications:
# Copyright (c) 2026 Hedele Zeng
#
# Modified for Mural-VISTA:
# 1.Highlight the picked seeds
# 2.Keep the SourceSeed visiable during the TargetSeed selection
# 3.Export the picked seeds coordinates 
# 
...
class vmtkPickPointSeedSelector(vmtkSeedSelector):
    ...
    def Execute(self):
        ...
        # --- begin custom seed‐highlight actor ---    # <--- New member
        sourceSeedPolyData = vtk.vtkPolyData()
        sourceSeedPolyData.DeepCopy(self.PickedSeeds)

        customSphere = vtk.vtkSphereSource()
        customSphere.SetRadius(self._Surface.GetLength()*0.01)
        customGlyph = vtk.vtkGlyph3D()
        customGlyph.SetSourceConnection(customSphere.GetOutputPort())
        customGlyph.SetInputData(sourceSeedPolyData)
        customGlyph.SetScaleModeToDataScalingOff()
        customGlyph.SetScaleFactor(1.0)
        customMapper = vtk.vtkPolyDataMapper()
        customMapper.SetInputConnection(customGlyph.GetOutputPort())
        self.CustomActor = vtk.vtkActor()
        self.CustomActor.SetMapper(customMapper)
        self.CustomActor.GetProperty().SetColor(0.0,1.0,0.0)  # e.g. green
        self.CustomActor.PickableOff()
        self.vmtkRenderer.Renderer.AddActor(self.CustomActor)
        # --- end custom seed‐highlight actor ---     # <--- New member
...
class vmtkCenterlines(pypes.pypeScript):
    ...
    def __init__(self):
        ...
        self.PickedSeedsSource = None  # <--- New member
        self.PickedSeedsTarget = None  # <--- New member
        ...
        self.SetOutputMembers([
            ...
            ['VoronoiDiagram','voronoidiagram','vtkPolyData',1,'','','vmtksurfacewriter'],
            ['PickedSeedsSource', 'pickedseedssource', 'vtkPolyData', 1, '', 'the vtkPolyData of picked seed points source', 'vmtkcenterlines'],   # <--- New member
            ['PickedSeedsTarget', 'pickedseedstarget', 'vtkPolyData', 1, '', 'the vtkPolyData of picked seed points target', 'vmtkcenterlines'],   # <--- New member
            ['PoleIds','poleids','vtkIdList',1]])
    ...
    def Execute(self):
        ...
        if self.SeedSelector:
            ...
            sourceIds = [inletSeedIds.GetId(i) for i in range(inletSeedIds.GetNumberOfIds())] # <--- New member
            targetIds = [outletSeedIds.GetId(i) for i in range(outletSeedIds.GetNumberOfIds())] # <--- New member
            self.PickedSeedsSource = [centerlineInputSurface.GetPoint(pid) for pid in sourceIds] # <--- New member
            self.PickedSeedsTarget = [centerlineInputSurface.GetPoint(pid) for pid in targetIds] # <--- New member
